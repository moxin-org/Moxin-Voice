...
//!     ↓
//! [Pre-Encoder LayerNorm] - Applied before transformer layers
//!     ↓
//! [Transformer Encoder] - 12 layers with post-norm attention and FFN
//!     ↓
...
//! 2. **Feature extractor**: Only layer 0 has GroupNorm(512,512)=instance norm, layers 1-6 don't
//! 3. **Positional conv**: Weight normalization with `w = g * v / ||v||`, groups=16, kernel=128
//! 4. **Pre-encoder LayerNorm**: Applied after positional embeddings, before transformer layers
//! 5. **Encoder layers**: Post-norm (LayerNorm after residual addition within each layer)
//!
...
pub struct ConvLayer {
    ...
    pub conv: nn::Conv1d,
    #[param]
    pub norm: Option<nn::GroupNorm>,  // Only layer 0 has GroupNorm (512, 512) - instance norm
    pub use_gelu: bool,
}
...
impl ConvLayer {
    pub fn new(
        ...
        // Note: conv has no bias in HuBERT feature extractor
        ...
        // HuBERT uses GroupNorm(512, 512) which is instance normalization
        // Each channel is normalized independently
        let norm = if use_norm {
            Some(nn::GroupNormBuilder::new(out_channels, out_channels)
                .eps(1e-5)
                .affine(true)
...
impl PosConvEmbed {
    ...
    fn compute_weight(&self) -> Result<Array, Exception> {
        ...
        //
        // PyTorch weight_norm with dim=2 means:
        // - Norm is computed over ALL dimensions EXCEPT dim 2 (kernel)
        // - For each kernel position k, compute norm across all (out, in) pairs
        // - Result shape: [1, 1, 128]

        let v = self.weight_v.as_ref();
        let g = self.weight_g.as_ref();

        // Reshape v to [kernel, out*in]: [768, 48, 128] -> [128, 768*48]
        // Then compute norm over the second dimension
        let v_transposed = transpose_axes(v, &[2, 0, 1])?;  // [128, 768, 48]
        let v_flat = v_transposed.reshape(&[self.kernel_size, -1])?;  // [128, 36864]

        // Compute L2 norm for each kernel position
        let v_sq = v_flat.multiply(&v_flat)?;
        let v_norm_sq = ops::sum_axis(&v_sq, 1, true)?;  // [128, 1]
        let v_norm = ops::sqrt(&v_norm_sq.add(array!(1e-7))?)?;

        // Reshape norm back to [1, 1, 128]
        let v_norm = v_norm.reshape(&[1, 1, self.kernel_size])?;

        // Normalize: v / ||v|| where ||v|| broadcasts from [1, 1, 128]
        let v_normalized = v.divide(&v_norm)?;  // [768, 48, 128]

        // Scale by g: g is [1, 1, 128]
        let weight = v_normalized.multiply(g)?;  // [768, 48, 128]
...
impl Module<&Array> for HuBertEncoder {
    ...
    fn forward(&mut self, audio: &Array) -> Result<Self::Output, Self::Error> {
        ...
        // Pre-encoder layer norm (applied BEFORE encoder layers, not after)
        // This matches HuggingFace HuBert where encoder.layer_norm is applied
        // after positional embeddings but before the transformer layers
