...
fn send_transcription(node: &mut DoraNode, text: &str) -> Result<()> {
    let arr = vec![text.to_string()].into_arrow();
    node.send_output("transcription".into(), BTreeMap::new(), arr)
            ...
            Event::Input { id, data, metadata } => {
                ...
                let samples_16k = if sample_rate != 16000 {
                    ...
                    match qwen3_asr_mlx::audio::resample(&samples, sample_rate, 16000) {
                        ...
                        Err(e) => {
                            ...
                            tracing::error!("{}", msg);
                            let _ = send_log(&mut node, &msg);
                            let _ = send_transcription(&mut node, "");
                            continue;
                        }
                    }
                } else {
                ...
                match model.transcribe_samples(&samples_16k, &language) {
                    Ok(text) => {
                        ...
                        send_transcription(&mut node, &text)?;
                    }
                    Err(e) => {
